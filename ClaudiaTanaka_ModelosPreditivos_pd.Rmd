...
Algumas variáveis que especificam os tipos de serviços assinados (ex. `StreamingMovies`) são multicategóricas mas deveriam ser binárias. A informação adicional sobre se o cliente tem ou não serviço de internet ou de telefone já está contida nas variáveis `PhoneService` e `InternetService`. Torná-las binárias evita redundância e simplifica o modelo.
...
Realiza uma análise explotratória dos dados (EDA) para entender melhor as variáveis e suas relações com a variável target `Churn`.
...
```{r demog, echo=FALSE, fig.width=4, fig.height=4, fig.cap="*Gráfico das variáveis demográficas versus Churn.*"}
df1 |> 
...
```{r services, echo=FALSE, fig.asp=.9, fig.width=4, fig.cap="*Gráfico das variáveis de serviços assinados versus Churn.*"}
...
Porém, a diferença na propensão ao cancelamento entre clientes com ou sem cada um desses serviços é sutil, exceto no caso de `OnlineSecurity` e `TechSupport`.
...
```{r account, echo=FALSE, fig.asp=1.5, fig.width=5, fig.cap="*Gráfico das variáveis de informações da conta versus Churn.*"}
...
O [Capítulo 10. Resampling for Evaluating Performance](https://www.tmwr.org/resampling#resampling) do livro Tidy Modeling with R ([Kuhn e Silge (2023)](#kuhn2023) explica os fundamentos da validação cruzada.

Algumas técnicas de modelagem (como regressão linear/logística, análise discriminante e outros) são considerados modelos de alto viés pois não são altamente adaptáveis aos tipos de padrões que podem ser encontrados nos dados. Porém muitos modelos como redes neurais, random forest, têm baixo viés, o que significa que podem reproduzir relacionamentos altamente complexos de vários tipos de padrões encontrados em um conjunto de dados. Isso pode resultar no modelo reproduzir quase exatamente os padrões (incluindo ruídos) encontrados do conjunto de treino e ter baixa performance quando fizer previsão sobre novas observações. Um modelo de baixo viés treinado em certo conjunto de dados fornecerá previsões otimistas para estes dados de treino, mas pode não funcionar tão bem para novas observações. Isso é conhecido como *overfitting*. 

Assim, para comparar a performance entre modelos de maneira justa é necessário avaliar o desempenho de cada modelo com os dados de treinamento antes de escolher o modelo final que será testado sobre os dados separados para especificamente para teste. 

Uma solução popular para particionamento justo dos dados é avaliar o desempenho em uma amostra dos dados reservada à qual o modelo não seja exposto durante sua construção – ou seja, uma separação dos dados de treino em subconjunto de "análise" e subconjunto de "validação".

O particionamento pode ser feito de várias maneiras, a mais comum sendo a partição aleatória da amostra em subconjuntos de análise e teste por métodos de reamostragem. Métodos de reamostragem são sistemas de simulação empírica que emulam o processo de usar alguns dados para treino e dados diferentes para teste. A maioria dos métodos de reamostragem são iterativos, o que significa que esse processo é repetido várias vezes. O diagrama na Figura a seguir ilustra como os métodos de reamostragem geralmente operam.
...
## Balanceamento de classes {#split}
...
Assim, a EDA pode ajudar a formar hipóteses sobre a escolha das variáveis a serem usadas no modelo, a escolha das técnicas de modelagem a serem testadas (Regressão Logística, Decision Tree, Random Forest, Redes Neurais, etc.), escolhas sobre como os dados devem ser transformados para a modelagem (*feature engineering*) para atingir o objetivo predeterminado.

Essa etapa também pode ajudar na limpeza dos dados, incluindo escolhas sobre como lidar com observações duplicadas, valores faltantes, outliers, multicolinearidade entre as variáveis. 
...
Como demonstrado na Figura \@ref(fig:processo), a análise de dados é um processo iterativo. A EDA pode levar a mais perguntas e a mais exploração de dados. A análise de dados é um processo de descoberta e aprendizado.

\
### Preprocessamento

Nessa etapa deve-se definir a estratégia de particionamento dos dados a ser adotada para a validação do modelo. Os dados serão separados em treino e teste? Será aplicada alguma metodologia de reamostragem? Como mencionado na pergunta [4.1 Validação cruzada](#validacao-cruzada), pode-se escolher algum tipo de validação cruzada, além de outras metodologias desenhadas para mitigar o *overfitting* e eventuais problemas específicos de tamanho ou distribuição dos dados da amostra de treino.
...
\
### Documentar e comunicar conclusões
...
Após a etapa de [definição do objetivo](#objetivo), [análise exploratória dos dados](#eda) e particionamento dos dados para [treino e teste](#split), a seguir escolhemos as métricas de performance para comparar os modelos entre si.
...
performance_metrics <- 
  metric_set(
    ...
    f_meas, precision, # <== f_meas = F1-score
  )
```
...
*Feature Engineering* transforma os dados para modelagem ([etapa 4](#etapas))
...
Define especificações/ hiperparâmetros aplicáveis e ajusta (treina) o modelo ([etapa 5](#etapas)).
...
Gera previsões sobre a base de teste e calcula medidas de performance do modelo. ([etapa 6](#etapas))
...
lgreg_res_roc_curve <- roc_curve(lgreg_test, truth=Churn, .pred_Churned)

# Extrai coeficientes de regressão
lgreg_res_coefs <- 
  tidy(lgreg_fit) |> 
  mutate(
    lg_significance = p.value < 0.05,
    lim.inf = estimate - qt(p=.95/2, df=nrow(df1_train)-1, lower.tail=F) * std.error,
    lim.sup = estimate + qt(p=.95/2, df=nrow(df1_train)-1, lower.tail=F) * std.error
  )
```
...
Reservaremos a análise dos resultados para a [pergunta seguinte](#resultados) que fára a comparação entre todos os modelos. Por ora, apenas ressaltamos que a acurácia do modelo de regressão logística está razoável e o `roc_auc` parece indicar um balanceamento aceitável entre as taxas de acerto de positivos e negativos.
...
Coeficientes da regressão: Uma vantagem do modelo de regressão logística é que os coeficientes estimados podem ser interpretados diretamente como a relação (linear, nesse caso) entre as variáveis explicativas e a probabilidade de ocorrência do evento na variável de saída. 

A seguir, apresentamos os coeficientes estimados e seus intervalos de confiança.

```{r lgreg_res_coefs, echo=FALSE, fig.cap="Estimativa dos coeficientes das variáveis preditoras da regressão logística ordenados de acordo com a significância e magnitude.", fig.width=6, fig.asp=1}
lgreg_res_coefs |> 
  filter(term != "(Intercept)") |>
  arrange(desc(lg_significance), desc(abs(estimate))) |> 
  mutate(id = row_number(),
         term = fct_reorder(term, -id),
         lg_significance = factor(lg_significance, labels=c("p < 0.05", "p > 0.05"))) |> 
  ggplot(aes(y=term, x=estimate, fill=lg_significance)) +
  geom_col() +
  geom_errorbar(aes(xmin=lim.inf, xmax=lim.sup), width=.2, color="darkgrey") +
  scale_fill_brewer(palette="Set2", direction=-1) +
  labs(title="Estimativa dos coeficientes da regressão logística",
       subtitle="Ordenados por significância (0,05) e magnitude",
       x="Estimativa", y=NULL)
```

\

Na análise exploratória, havíamos levantado as hipóteses de que as seguintes variáveis poderiam ser candidatas a ser indicadoras da propensão ao cancelamento:

* `SeniorCitizen` (vide  Figura \@ref(fig:demog)),
* `InternetService`, `OnlineSecurity` e `TechSupport` (vide Figura \@ref(fig:services)),
* `Contract` (tipo de contrato), `PaperlessBilling`, `PaymentMethod` e `Tenure` (vide \@ref(fig:account))

Olhando a Figura \@ref(fig:lgreg_res_coefs), não ter serviços de internet contratados aparece como a maior indicadora de aumento da propensão ao cancelamento enquanto ter fibra ótica (também da variável `InternetService`) parece estar associado a redução da propensão ao cancelamento, embora eu inicialmente achasse que a direção da influência seria oposta pelos gráficos de proporção da Figura \@ref(fig:services). 

Assim, o modelo parece confirmar nossas suspeitas em relação a `InternetService`, `OnlineSecurity`, `TechSupport`, `Contract` enquanto as variáveis `SeniorCitizen`, `PaperlessBilling`, `PaymentMethod` e `Tenure` não parecem ter tanta influência na propensão ao cancelamento quanto eu imaginava inicialmente.

O modelo também encontrou valor informativo na variável preditora `n.internet` que contabiliza a quantidade de serviços adicionais de internet contratados pelo cliente. 

Talvez seja interessante investigar a interação entre as variáveis `n.internet` e as variáveis dummy de cada um dos serviços de internet contratados pois, especialmente no caso das variáveis com coeficientes de menor magnitude (`StreamingTV`, `OnlineBackup`, `DeviceProtection`) talvez haja alguma redundância ou efeito de multicolinearidade.
...
# Grava a curva ROC do modelo
tree_roc_curve <- roc_curve(tree_test, truth=Churn, .pred_Churned)
```


\

Reservaremos a análise dos resultados para a [pergunta seguinte](#resultados) que fára a comparação entre todos os modelos. Por ora, apenas ressaltamos que a acurácia da Árvore de Decisão está razoável e o `roc_auc` parece indicar pequeno balanceamento entre as taxas de acerto de positivos e negativos.

```{r}
df4_performance |> filter(model=="DecTree0")
```
...
```{r dectree, fig.cap="Árvore de decisão gerada pelo modelo de classificação.", out.width=300, fig.align="center"}
extract_fit_engine(tree_fit) |> 
  rpart.plot::rpart.plot(yes.text="SIM", no.text="NÃO", roundint=FALSE)
```

Apesar de ser um modelo muito instável por ser muito sensível a pequenas alterações dos dados de treino, vale notar que a Árvore de Decisão, assim como a Regressão Logística, identificou a contratação de fibra ótica como um dos indicadores de propensão ao cancelamento. Por outro lado, as variáveis `Contract` sendo renovado mensalmente e quantidade de meses da assinatura (`Tenure`) não apareceram entre as mais relevantes na Regressão Logística. 

No caso de `Tenure` talvez faça sentido experimentar rodar a Regressão Logística com uma categorização da quantidade de meses que separe < 6 meses de outras faixas da variável para ver se essa variável se torna mais relevante naquele modelo e ajuda a aumentar a performance do modelo. 
...
# Grava a curva ROC do modelo
rf_roc_curve <- roc_curve(rf_test, truth=Churn, .pred_Churned)
```


\

Reservaremos a análise dos resultados para a [pergunta seguinte](#resultados) que fára a comparação entre todos os modelos. Por ora, apenas ressaltamos que a acurácia da Random Forest está razoável e o `roc_auc` parece indicar balanceamento aceitável entre as taxas de acerto de positivos e negativos.
...
Infelizmente não é possível interpretar diretamente a magnitude da importância relativa das variáveis em um modelo Random Forest como é possível fazer no modelo de Regressão Logística. A unidade de medida aqui não tem um significado intrínseco. Essa medida serve apenas para comparar a importância entre as variáveis.

O modelo de Random Forest, confirmou as variáveis `Contract` (e.g. bianual, renovado mensalmente) e `InternetService` (e.g. fibra ótica) como bons indicadores de propensão ao cancelamento e também confirmou a variável `Tenure` que havia aparecido na árvore de decisão.

Por outro lado, as variáveis numéricas `MonthlyCharges` e `TotalCharges` surgem como novidade entre as mais relevantes. Talvez fizesse sentido investigar também a categorização dessas variáveis em faixas de valores para ver se elas se tornam mais relevantes na Regressão Logística e se melhoram a capacidade preditiva daquele modelo.
...
# Grava a curva ROC do modelo
nn_roc_curve <- roc_curve(nn_test, truth=Churn, .pred_Churned)
```
...
```{r}
df4_performance |> filter(model=="NNet0")
```
...
## Melhor modelo {#resultados}

**Em relação à questão anterior, qual o modelo deveria ser escolhido para uma eventual operação.**

\

A seguir visualizaremos os resultados, lembrando que as principais as medidas de performance são:

* **Acurácia** (`accuracy`): proporção de acertos sobre o total de previsões.

* **Recall** (também conhecido como `sensitivity` ou *True Positive Rate*): proporção de acertos positivos (TP) sobre todas as observações positivas (TP + FN).

* **Especificidade** (`specificity` ou *True Negative Rate*): proporção de acertos negativos (TN) sobre todas as observações negativas (TN + FP).

* **Precisão** (`precision`): proporção de acertos positivos (TP) em relação a todas as previsões positivas (TP + FP).

* **F1-score** (`f_meas`): média harmônica entre precisão e recall e, portanto, é uma medida que se preocupa mais com a performance em termos de acertos positivos (TP). Não é uma boa medida para quem busca o balanceamento entre acertos positivos e negativos.

* **AUC da curva ROC**: Cálculo da área abaixo da curva ROC, que mostra a relação entre a taxa de verdadeiros positivos (sensibilidade) e a taxa de falsos positivos para diferentes pontos de corte. Mede a capacidade do modelo de distinguir entre as classes (positivo vs negativo).
...
Na nossa análise, um resultado Positivo $P$ é a previsão de que o cliente irá cancelar o serviço (`Churned`) e um resultado Negativo $N$ é a previsão de que o cliente não irá cancelar o serviço (`Renewed`).

Em termos de acurácia, nota-se que todos os modelos ficaram com acurácia próxima, sendo a Regressão Logística o modelo com menor acurácia (0,73).

O modelo de Random Forest e a Regressão Logística se destacam com a melhor medida de `roc_auc` e seus valores ficam em torno de 84%, o que é um nível bem superior aos 50% (previsão aleatória) e indica um melhor balanceamento entre as classes previstas. 

Avaliando a diferença entre as taxas de acerto positivo e negativo, a *True positive rate* (`recall`) é maior que a *True negative rate* (`specificity`) em todos os modelos, exceto na rede neural. De fato, olhando a matriz de confusão é fácil notar que o modelo de rede neural se destaca no acerto de negativos 1002 (TN) porém tem um desempenho bem inferior no acerto de positivos com apenas 306 (TP), mesmo com o rebalanceamento da classe positiva que era minoritária na base de treino (~26P/74N).

O F1-score (`f_meas`) é baixo em todos os modelos, mas especialmente no modelo de Rede Neural. Isso indica que os modelos não estariam tão bons em acertar as previsões positivas (`Churned`), que é a classe minoritária. Isso é evidenciado pela baixa taxa de precisão ($TP/(TP+FP)$), em torno de 50%.

Se eu tivesse que escolher entre um destes modelos eu optaria pelo modelo de Random Forest `RndFor0` que tem a melhor acurácia e um `roc_auc` equilibrado e equivalente ao do melhor `roc_auc`. Comparado com os outros modelos, ele ganha mais na *True Negative rate* do que perde na *True positive rate*.

Mas acho que há oportunidade para experimentar algumas transformações de variáveis para ver se a performance dos modelos aumenta conforme comentado na análise dos coeficientes da regressão logística, na análise da árvore de decisão e na análise da importância das variáveis do modelo de Random Forest.

Além disso, seria interessante tentar fazer um `grid search` para refinar os hiperparâmetros dos modelos de Random Forest e Rede Neural.

Acredito que esses passos poderiam melhorar a performance dos modelos.



```{r performanceALL, fig.cap="Comparação das métricas de performance entre os modelos", fig.width=6}
df4_performance |> 
  mutate(
    model = factor(model, levels=c("LogReg0", "DecTree0", "RndFor0", "NNet0")),
    .metric = factor(.metric, levels=c("roc_auc", "recall", "specificity",
                                       "accuracy", "f_meas", "precision"))
  ) |>
  ggplot(aes(x=model, y=.estimate, group=.metric)) +
  geom_line(aes(color=.metric), linewidth=.7) + geom_point(aes(color=.metric)) +
  geom_text(aes(label=scales::number(.estimate, accuracy=.001)), vjust=-1, size=3) +
  ...
  labs(title="Comparação de medidas de performance entre os modelos", 
       subtitle="Ordenados pela sequência de geração dos modelos",
       x=NULL, y=NULL)
```
...
p1 <- pltROC(lgreg_res_roc_curve, "LogReg0")
...
\
Bruce, P.; Bruce, A.; Gedeck, P. (2020). [*"Practical Statistics for Data Scientists"*]{.underline}. O’reilly Media, Inc. [ISBN-13: 978-1492072911. ]{#bruce2020}

\
Kuhn, M.; Silge, J. (2023). [*"Tidy Modeling with R: A Framework for Modeling in the Tidyverse"*]{.underline}. O’reilly Media, Inc. ISBN-13: 978-1492096450. [[online](https://www.tmwr.org/) ]{#kuhn2023}

\
Shmueli, G. (2010). [*"To Explain or to Predict?"*]{.underline}. Journal of Statistical Science, Institute of Mathematical Statistics [online] v. 25, n. 3, p. 289–310. DOI: 10.1214/10-STS330. [[online](https://projecteuclid.org/euclid.ss/1294167961) ]{#shmueli2010}

\
Wickham, H.; Çetinkaya-Rundel, M.; Grolemund, G. (2023). [*"R for Data Science, 2nd Edition"*]{.underline}. O'Reilly Media, Inc. ISBN-13: 978-1492097365. [[online](https://r4ds.hadley.nz/) ]{#wickham2023}
...
\
Geisser, S. (1993). [*"Predictive Inference: An Introduction"*]{.underline}. Chapman and Hall, Londres. DOI: 10.1201/9780203742310. [[online](https://doi.org/10.1201/9780203742310) ]{#geisser1993}

\
Kuhn, M.; Johnson, K. (2013). [*"Applied Predictive Modeling"*]{.underline}. Springer. ISBN-13: 978-1461468486. [[online](https://link.springer.com/book/10.1007/978-1-4614-7472-3) ]{#kuhn2013}

\
Kuhn, M. (2021). [*"Feature Engineering and Selection: A Practical Approach for Predictive Models"*]{.underline}. Chapman and Hall/CRC. ISBN-13: 978-0367530679. [[online](https://www.routledge.com/Feature-Engineering-and-Selection-A-Practical-Approach-for-Predictive-Models/Kuhn/p/book/9780367530679) ]{#kuhn2021}

\
Lange, C. (2024). [*"Practical Machine Learning with R, 1st Edition"*]{.underline}. Chapman & Hall. ISBN-13: 978-1032434056. [[online](https://www.routledge.com/Practical-Machine-Learning-with-R-Tutorials-and-Case-Studies/Lange/p/book/9781032434056) ]{#lange2024}
