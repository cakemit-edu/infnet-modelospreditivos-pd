...
```{r}
x <- filter(df0, is.na(TotalCharges)) |>
  select(-customerID) |>
  mutate(
...
::: {.alert .alert-danger}
NOTA: INCLUIR VARIÁVEL DE CONTAGEM DE SERVIÇOS ASSINADOS
CHECAR SE EXCLUSÃO DE MISSINGS VAI ALTERAR ANÁLISES DE EDA
...
```{r echo=FALSE, fig.asp=1.7, fig.width=5}
ggbivariate(df1 |> select(PhoneService:StreamingMovies, Churn), 
            outcome="Churn", 
            rowbar_args = list(colour = "white",
                               size = 4,
                               label_format = scales::label_percent(accurary = 1)),
            # types = list(comboVertical="autopoint"),
            title = "Churn x Serviços assinados") +
...
```{r echo=FALSE, fig.asp=1.2, fig.width=5}
...
ggpairs(df1 |> select(is.numeric, Churn), aes(color=Churn, fill=Churn),
        ...
        title="Correlações entre as variáveis numéricas considerando Churn") +
  scale_fill_brewer(palette="Set1") +
  scale_color_brewer(palette="Set1")
```
