...
```{r bidemog, echo=FALSE, fig.asp=1, fig.width=4, fig.cap="*Gráfico das variáveis demográficas versus Churn.*"}
...
```{r include=FALSE}
comInternetTelefone <- round(nrow(filter(df1,PhoneService=="Yes",InternetService!="No"))/nrow(df1)*100)
telefonia <- round(nrow(filter(df1,PhoneService=="Yes"))/nrow(df1)*100)
multiplelines <- round(nrow(filter(df1,MultipleLines=="Yes"))/nrow(df1)*100)
internetService <- round(nrow(filter(df1,InternetService=="Fiber optic"))/nrow(df1)*100)
dsl <- round(nrow(filter(df1,InternetService=="DSL"))/nrow(df1)*100)
n.internet <- round(nrow(filter(df1,InternetService!="No" & n.internet > 0)) / nrow(filter(df1,InternetService!="No"))*100)
n.streaming <- round(df1 |> filter(StreamingTV=="Yes"|StreamingMovies=="Yes") |> nrow() / nrow(df1)*100)
```


Sendo uma empresa de telecomunicações, todos os clientes têm `PhoneService` e/ou `InternetService` contratado. Especificamente verifica-se que a maioria (`r comInternetTelefone`%) dos clientes têm tanto internet quanto telefonia.

Em relação ao padrão de serviços de telefonia contratados, como `r telefonia`% dos clientes têm telefonia e `r multiplelines`% dos clientes têm múltiplas linhas, quase a metade dos clientes de telefonia tem mais de uma linha.

Clientes em fibra ótica são um pouco mais frequentes que clientes com DSL. Dos clientes que assinam internet com a empresa, `r internetService`% contrataram fibra ótica e `r dsl`% contrataram DSL.

Verifica-se no dataset também que `r n.internet`% dos clientes com internet têm algum tipo de serviço adicional contratado. Como se vê na Figura abaixo, a maioria dos serviços adicionais não chega a ser contratado por metade dos clientes com internet, exceto os serviços de *streaming*.

Os serviços adicionais mais frequentes são `StreamingTV` e `StreamingMovies`. De fato, dos clientes com serviços de internet, verifica-se que `r n.streaming`% contratam TV ou Movies ou ambos. Seria interessante conversar com a equipe de negócios para checar a diferença entre esses dois serviços e verificar se faria sentido juntar os clientes destes serviços em uma única variável, caso isso melhorasse a performance do modelo em produção.
...
```{r include=FALSE}
# rm(comInternetTelefone, telefonia, multiplelines, internetService, dsl, n.internet, n.streaming)
# gc()
```
...
Selecionar as métricas de classificação e probabilidade para avaliar a performance de todos os modelos.

```{r}
performance_metrics <- 
  metric_set(accuracy, 
             roc_auc, sensitivity, specificity,
             f_meas, precision, recall, # <== F1-score
             brier_class)
```
...
df3_conf_matrix <- conf_mat(tree_test, truth=Churn, estimate=.pred_class) |> 
  tidy() |> 
  tidyConfMat() |> 
...
df4_performance |> filter(.metric %in% c("accuracy", "precision", "recall", "f_meas"))
```
...
A seguir vamos verificar a importância das variáveis.
